    ...
    if (action === "register") {
      ...
      const { messageID: regMsgID } = await createReply(api, {
        ...
        callback: async ({ api, event, end, reply }: any) => {
          ...
          if (fresh.name) { await api.sendMessage(styled("Fate Registration", "🛑", "You are already registered."), event.threadID, event.messageID); return; }
    ...
    if (action === "battle") {
      ...
      if (win && (s.npCharge >= 100 || isDev)) {
        await createReply(api, {
          ...
          keepAlive: false,
          authorID: senderID,
          callback: async ({ api, event, end }: any) => {
            end();
            ...
            if (reply === "unleash") {
              ...
              await api.sendMessage(styled("Fate Battle", "💾", `${s.npName} saved for later.`), event.threadID, event.messageID);
            }
          },
        });
      } else {
        ...
        await createReply(api, {
          ...
          callback: async ({ api, event, end }: any) => {
            ...
            if (gw.participants.length >= GW_MAX) {
              await api.sendMessage(styled("Holy Grail War", "🛑", `War is full (${GW_MAX}/${GW_MAX}).`), event.threadID, event.messageID);
              end(); return;
            }
            const joiner = await db.db("fate_servants").findOne({ userID: event.senderID.toString() });
            if (!joiner?.name) {
              await api.sendMessage(styled("Holy Grail War", "⚠️", "You must register first. Use fate register <n>."), event.threadID, event.messageID);
              return;
            }
    ...
    if (action === "raid") {
      ...
      await createReply(api, {
        ...
        callback: async ({ api, event, end }: any) => {
          ...
          const joiner = await db.db("fate_servants").findOne({ userID: event.senderID.toString() });
          if (!joiner?.name) {
            await api.sendMessage(styled("Raid", "⚠️", "You must register first. Use fate register <n>."), event.threadID, event.messageID);
            return;
          }
