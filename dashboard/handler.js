...
module.exports = function mountDashboard(app) {
  ...
  app.post('/social/myday', async (req, res) => {
    ...
    if (sizeBytes > 10 * 1024 * 1024) return res.status(413).json({ ok: false, error: 'File too large. Max 10MB.' });
    try {
      const id = newId();
      const now = Date.now();
  ...
  app.post('/social/post', async (req, res) => {
    ...
    if (mediaData && Buffer.byteLength(mediaData, 'base64') > 10 * 1024 * 1024)
