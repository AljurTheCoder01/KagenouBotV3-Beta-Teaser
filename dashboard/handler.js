  ...
  async function runGuestCommand(uid, input, replyToMessageID) {
    ...
    if (trimmed.startsWith(prefix)) {
      ...
      if (global.nonPrefixCommands?.size) {
        ...
        const firstWord = inputLower.split(/\s+/)[0] || "";
        for (const [key, cmd] of global.nonPrefixCommands) {
          if (!(cmd.config?.nonPrefix === true || cmd.nonPrefix === true)) continue;
          const name = (cmd.config?.name || cmd.name || key || "").toLowerCase();
          if (firstWord !== name) continue;
          if (global.commands?.has(name)) continue;
          command = cmd;
          cmdName = name;
