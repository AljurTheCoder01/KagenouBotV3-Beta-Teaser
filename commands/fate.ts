...
🔱 Equipped Weapons:\n${equippedWeapons}`
    ...
    if (action === "bond") {
      ...
      if ((s.bondLevel || 0) >= 20) { await api.sendMessage(styled("Bond Training", "💫", `${s.servantCard} bond is at MAX LEVEL (20)!\nAll stat bonuses are fully applied.`), threadID, messageID); return; }
    ...
    if (action === "daily") {
      ...
      s.gems = (s.gems || 0) + gemsReward; s.goldCoins = (s.goldCoins || 0) + coinsReward; s.exp = (s.exp || 0) + expReward;
    ...
    if (action === "singularity") {
      ...
      if (idx >= SINGULARITIES.length) { await api.sendMessage(styled("Singularity", "🌟", "You have cleared all Singularities! Grand Order complete!"), threadID, messageID); return; }
      ...
      if (win) {
        s.singularityProgress = idx + 1; s.exp = (s.exp || 0) + sing.expReward;
        s.goldCoins = (s.goldCoins || 0) + sing.coinsReward; s.holyGrails = (s.holyGrails || 0) + 1;
        s.level = calcLevel(s.exp);
        const newTitles = checkAndGrantFateTitles(s);
        await saveServantData(db, senderID.toString(), s);
    ...
    if (action === "shop") {
      ...
      if (shopSub === "hero") {
        ...
        global.registerEnkiduListener(heroMsgID, async ({ api, event }: any) => {
          ...
          const hid = parts?.[0]; const confirm = parts?.[1]?.toLowerCase();
      ...
      if (shopSub === "list") {
        ...
        global.registerEnkiduListener(listMsgID, async ({ api, event }: any) => {
          if (event.senderID !== senderID) return;
          const parts = event.body?.trim().toUpperCase().split(/\s+/);
          const shopId = parts?.[0]; const confirm = parts?.[1]?.toLowerCase(); const qty = parseInt(parts?.[2] || "1") || 1;
          ...
          if (entry.type === "weapon") {
            ...
            fresh.stats.atk = (fresh.stats.atk || 0) + w.atkBonus; fresh.stats.def = (fresh.stats.def || 0) + w.defBonus; fresh.stats.mana = (fresh.stats.mana || 0) + w.manaBonus;
            const newTitles = checkAndGrantFateTitles(fresh);
            await saveServantData(db, senderID.toString(), fresh);
            ...
            if (curUpg >= 10) { await api.sendMessage(styled("Item Shop", "🔱", `${w.name} is already at MAX upgrade (Lv.10)!`), event.threadID, event.messageID); return; }
            ...
            const atkAdd = Math.floor(w.atkBonus * bonusMult); const defAdd = Math.floor(w.defBonus * bonusMult); const manaAdd = Math.floor(w.manaBonus * bonusMult);
            fresh.stats.atk = (fresh.stats.atk || 0) + atkAdd; fresh.stats.def = (fresh.stats.def || 0) + defAdd; fresh.stats.mana = (fresh.stats.mana || 0) + manaAdd;
            const newTitles = checkAndGrantFateTitles(fresh);
            await saveServantData(db, senderID.toString(), fresh);
            await api.sendMessage(styled("Item Shop", "🔱", `${w.name} upgraded to Lv.${fresh.weaponUpgrades[entry.ref]}/10!\n+${atkAdd} ATK | +${defAdd} DEF | +${manaAdd} Mana\nCost: ${upgCost.toLocaleString()} 🪙\nGold remaining: ${fresh.goldCoins.toLocaleString()} 🪙${titleLine(newTitles)}\n\nReply with another Shop ID + buy to keep shopping.`), event.threadID, event.messageID);
          }
        });
        return;
      }
    ...
    if (action === "buy") {
      ...
      if (isWeapon) {
        ...
        s.stats.atk = (s.stats.atk || 0) + w.atkBonus; s.stats.def = (s.stats.def || 0) + w.defBonus; s.stats.mana = (s.stats.mana || 0) + w.manaBonus;
        const newTitles = checkAndGrantFateTitles(s);
        await saveServantData(db, senderID.toString(), s);
    ...
    if (action === "use") {
      ...
      if (!useID || !POTION_ITEMS[useID]) { await api.sendMessage(styled("Use Item", "⚠️", "Usage: /fate use <potionID> <qty>\nExample: /fate use P001 2\nSee /fate shop for potion IDs."), threadID, messageID); return; }
      const owned = s.inventory.potions[useID] || 0;
      if (!isDev && owned < useQty) { await api.sendMessage(styled("Use Item", "❌", `You only have ${owned}× ${POTION_ITEMS[useID].name}.\nBuy more with /fate buy ${useID} <qty>.`), threadID, messageID); return; }
    ...
    if (action === "upgrade") {
      ...
      if (!upID || !WEAPON_ITEMS[upID]) { await api.sendMessage(styled("Weapon Upgrade", "⚠️", "Usage: /fate upgrade <weaponID>\nExample: /fate upgrade W005\nYou must own the weapon first."), threadID, messageID); return; }
      if (!s.inventory.weapons[upID] && !isDev) { await api.sendMessage(styled("Weapon Upgrade", "❌", `You don't own ${WEAPON_ITEMS[upID].name}.\nBuy it first with /fate buy ${upID} 1.`), threadID, messageID); return; }
      ...
      const w = WEAPON_ITEMS[upID]; const upgradeCost = isDev ? 0 : Math.floor(w.cost * 0.3 * (currentUpgrade + 1));
      ...
      const atkAdd = Math.floor(w.atkBonus * bonusMultiplier); const defAdd = Math.floor(w.defBonus * bonusMultiplier); const manaAdd = Math.floor(w.manaBonus * bonusMultiplier);
      s.stats.atk = (s.stats.atk || 0) + atkAdd; s.stats.def = (s.stats.def || 0) + defAdd; s.stats.mana = (s.stats.mana || 0) + manaAdd;
      const newTitles = checkAndGrantFateTitles(s);
      await saveServantData(db, senderID.toString(), s);
      await api.sendMessage(styled("Weapon Upgrade", "🔱", `${w.name} upgraded to Lv.${s.weaponUpgrades[upID]}/${MAX_UPGRADE}!\n\n+${atkAdd} ATK | +${defAdd} DEF | +${manaAdd} Mana\nCost: ${upgradeCost.toLocaleString()} 🪙\nGold remaining: ${s.goldCoins.toLocaleString()} 🪙${newTitles.length > 0 ? titleLine(newTitles) : ""}`), threadID, messageID);
      return;
    }
    ...
    if (action === "skill") {
      ...
      if (!skillSub || skillSub === "list") {
        const skillList = Object.entries(SKILL_DEFS).map(([id, sk]) => `${(s.skills || []).includes(id) ? "✅" : "🔒"} [${id}] ${sk.name} — ${sk.description}\n       Unlock: ${sk.unlockReq} | Cost: ${sk.cost.toLocaleString()} 🪙`).join("\n");
      ...
      if (skillSub === "learn") {
        ...
        if (!skID || !SKILL_DEFS[skID]) { await api.sendMessage(styled("Skills", "⚠️", "Usage: /fate skill learn <ID>\nExample: /fate skill learn SK01\nSee /fate skill list for IDs."), threadID, messageID); return; }
    ...
    if (action === "title") {
      ...
      if (!titleSub || titleSub === "list") {
        ...
        await api.sendMessage(styled("Servant Titles", "🎖️", `Active Title: ${active}\n\n${list}\n\nEquip: /fate title set <titleId>\nIDs: ${FATE_TITLES.map(t => t.id).join(", ")}`), threadID, messageID); return;
      }
      if (titleSub === "set") {
        const tid = args.slice(2).join("_").toLowerCase();
        if (!tid || !FATE_TITLES.find(t => t.id === tid)) { await api.sendMessage(styled("Servant Titles", "⚠️", "Invalid title ID. Use /fate title list to see available IDs."), threadID, messageID); return; }
    ...
    if (action === "chaldea") {
      const chaldeaSub = args[1]?.toLowerCase(); const chaldeaName = args.slice(2).join(" ").trim(); const col = db.db("fate_chaldeas");
    ...
    if (action === "grailwar") {
      const gwSub = args[1]?.toLowerCase(); const existing = activeGrailWars.get(threadID);
      if (gwSub === "start") {
        ...
        const GW_DURATION = 300; const GW_MAX = 13;
      ...
      if (gwSub === "begin") {
        ...
        if (existing.participants.length < 2) { await api.sendMessage(styled("Holy Grail War", "⚠️", `Need at least 2 participants. Current: ${existing.participants.length}/13.`), threadID, messageID); return; }
        const col = db.db("fate_servants");
        ...
        if (winnerData) {
          winnerData.holyGrails = (winnerData.holyGrails || 0) + 3; winnerData.exp = (winnerData.exp || 0) + 50000;
          winnerData.gems = (winnerData.gems || 0) + 20; winnerData.level = calcLevel(winnerData.exp); winnerData.grailWarWins = (winnerData.grailWarWins || 0) + 1;
    ...
    if (action === "raid") {
      const raidSub = args[1]?.toLowerCase(); const existingR = activeRaidsFate.get(threadID);
      if (raidSub === "start") {
        ...
        if (existingR.participants.length < 2) { await api.sendMessage(styled("Fate Raid", "⚠️", `Need at least 2 servants. Current: ${existingR.participants.length}/5.`), threadID, messageID); return; }
        const raidBosses = [
        ...
        const col = db.db("fate_servants"); let partyPow = 0; const names: string[] = [];
        ...
        for (const pid of existingR.participants) {
          const pd = await col.findOne({ userID: pid });
          if (pd) {
            pd.exp = (pd.exp || 0) + (raidWin ? 15000 : 2000); pd.goldCoins = (pd.goldCoins || 0) + (raidWin ? 5000 : 500);
            if (raidWin) pd.holyGrails = (pd.holyGrails || 0) + 1; pd.level = calcLevel(pd.exp);
      ...
      if (existingR) { await api.sendMessage(styled("Fate Raid", "ℹ️", `Raid open: ${existingR.participants.length}/5 servants. Reply 'join raid' to the original lobby message, or /fate raid start.`), threadID, messageID); return; }
      const RAID_DURATION = 300; const RAID_MAX = 5;
    ...
    if (action === "surge") {
      ...
      if (surgeSub === "start") {
        ...
        const isAuth = isDev || (global.config.admins && global.config.admins.map(String).includes(sid)) || (global.config.developers && global.config.developers.map(String).includes(sid)) || (global.config.vips && global.config.vips.map(String).includes(sid));
        ...
        setTimeout(() => { const sg = activeSurgesFate.get(threadID); if (sg && !sg.claimedBy) { activeSurgesFate.delete(threadID); api.sendMessage(styled("Grail Surge", "🏆", "The Holy Grail faded before anyone could claim it."), threadID); } }, 600000);
        return;
      }
      if (surgeSub === "enter") {
        ...
        surge.claimedBy = senderID.toString(); activeSurgesFate.set(threadID, surge);
        s.holyGrails = (s.holyGrails || 0) + 1; s.exp = (s.exp || 0) + 10000; s.goldCoins = (s.goldCoins || 0) + 2000; s.gems = (s.gems || 0) + 10; s.level = calcLevel(s.exp);
        const newTitles = checkAndGrantFateTitles(s);
        await saveServantData(db, senderID.toString(), s);
    ...
    if (action === "setstat") {
      const sid = senderID.toString();
      const isAuth = isDev || (global.config.admins && global.config.admins.map(String).includes(sid)) || (global.config.developers && global.config.developers.map(String).includes(sid));
